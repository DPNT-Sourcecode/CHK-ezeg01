...
class CheckoutSolution {
    ...
    checkout(skus) {
        ...
        const complexOffers = [
            ...
            { trigger: 'U', requiredQty: 4, freeItem: 'U', freeQty: 1 },
        ];
        ...
        // F promotion
        if(summedProducts.F) {
            const totalFs = summedProducts.F;
            const freeFs = Math.floor(totalFs / 3);
            summedProducts.F = totalFs - freeFs;
        }

        // U Promotion
        if (summedProducts.U) {
            const totalUs = summedProducts.U;
            const freeUs = Math.floor(totalUs / 4);
            summedProducts.U = totalUs - freeUs;
        }
